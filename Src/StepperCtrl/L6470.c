...
const static MotorParameterData_t L6470_MotorParameterInitData[L6470DAISYCHAINSIZE] =
  {
	{12.0, 400, 0.48, 12.0, 10000.0, 30000.0, 30000.0, 1500.0, 0.0, 15625.0, 200, 200,\
	   		200, 200, 200.5,392.1569e-3,  643.1372e-3,  643.1372e-3, 0,\
	    3.06*1000*1.10, 3.06*1000*1.00, MICROSTEP_1_8, 0xFF, 0x2E88},

    {12.0, 400, 0.48, 12.0, 10000.0, 30000.0, 30000.0, 1500.0, 0.0, 15625.0, 200, 200,\
    		200, 200, 200.5,392.1569e-3,  643.1372e-3,  643.1372e-3, 0,\
        3.06*1000*1.10, 3.06*1000*1.00, MICROSTEP_1_8, 0xFF, 0x2E88},

	{12.0, 400, 0.48, 12.0, 10000.0, 30000.0, 30000.0, 1500.0, 0.0, 15625.0, 12.0, 12.0,\
	  		12.0, 12.0, 200.5,392.1569e-6,  643.1372e-6,  643.1372e-6, 0,\
	    3.06*1000*1.10, 3.06*1000*1.00, MICROSTEP_1_8, 0xFF, 0x2E88},
...
/*The data structure for all further instances to stepper motor drivers.*/
StepperMotorDriverHandle_t L6470DrvMtx[L6470DAISYCHAINSIZE] =
{
    {
      0,
      &L6470Command,
...
static void L6470_ConfigureChips(void);


HAL_StatusTypeDef L6470_InitSPI()
{
    ...
    */
	  ...
	  GPIO_InitStruct.Pin = L6470_CS_PIN;
	  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	  GPIO_InitStruct.Speed = GPIO_SPEED_LOW;
	  GPIO_InitStruct.Pull = GPIO_PULLUP;
	  HAL_GPIO_Init(L6470_CS_PORT, &GPIO_InitStruct);

	  GPIO_InitStruct.Pull = GPIO_NOPULL;
	  GPIO_InitStruct.Pin = L6470_RESET_PIN;
	  HAL_GPIO_Init(L6470_RESET_PORT, &GPIO_InitStruct);

	  L6470_nCS_HIGH();
	  L6470_DISABLE();

      GPIO_InitStruct.Pin = L6470_SCK_PIN;
      GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
      GPIO_InitStruct.Pull = GPIO_NOPULL;
      GPIO_InitStruct.Alternate = GPIO_AF5_SPI4;
      GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
      HAL_GPIO_Init(L6470_SCK_PORT, &GPIO_InitStruct);

      GPIO_InitStruct.Pin = L6470_MOSI_PIN;
      HAL_GPIO_Init(L6470_MOSI_PORT, &GPIO_InitStruct);

	  GPIO_InitStruct.Pin = L6470_MISO_PIN;
	  HAL_GPIO_Init(L6470_MISO_PORT, &GPIO_InitStruct);

	  handleSPI.Instance = SPI4;
	  ...
	  handleSPI.Init.CRCPolynomial = 10;

	  handleSPI.Init.NSS = SPI_NSS_SOFT; //SPI_NSS_HARD_OUTPUT;
	  handleSPI.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32;  //SPI frequency is APB2 (84Mhz)


	  HAL_StatusTypeDef status = HAL_SPI_Init(&handleSPI);

	  L6470_ENABLE();

	  //configure motors to default values
	  L6470_ConfigureChips();
...
static void L6470_ConfigureChips(void)
{
	//these motors move 1.8 deg per full step, or 200 steps per revolution

  for (uint8_t id = 0; id < L6470DAISYCHAINSIZE; id++)
  {
    L6470_Config((StepperMotorDriverHandle_t*)(&L6470DrvMtx[id]), (MotorParameterData_t*)(&L6470_MotorParameterInitData[id]));

    //MotorHandle->Command->CheckStatusRegisterFlag()
  }
}

/**
  * @brief  Calculates the er of a number.
  * @param  base      the base
  * @param  exponent  the exponent
  * @retval er     the result as (base^exponent)
  * @note   There is not OVF control.
  */
/*
uint32_t (uint8_t base, uint8_t exponent)
{
  uint8_t i;
  uint32_t er = 1;
...
/**
  * @brief  ResetDevice command resets the device to er-up conditions.
  * 
  * @param  L6470_Id      The identifier of the L6470 target inside the daisy chain.
  */
...
/**
  * @brief  SoftHiZ command disables the er bridges (high impedance state)
...
/**
  * @brief  HardHiZ command immediately disables the er bridges (high impedance state).
  * 
  * @param  L6470_Id      The identifier of the L6470 target inside the daisy chain.
  */
...
void L6470_DaisyChainCommand(uint8_t* pL6470_DaisyChainSpiTxStruct, uint8_t* pL6470_DaisyChainSpiRxStruct)
{
  ...
  for(spibyte=0; spibyte < L6470MAXSPICMDBYTESIZE; spibyte++)
  {
    ...
    HAL_StatusTypeDef status = HAL_SPI_TransmitReceive(&handleSPI, (pL6470_DaisyChainSpiTxStruct+(spibyte * L6470DAISYCHAINSIZE)), (pL6470_DaisyChainSpiRxStruct+(spibyte * L6470DAISYCHAINSIZE)), L6470DAISYCHAINSIZE, 10);
    if (status != HAL_OK) BA_ERROR_HANLDER(BA_ERROR_L6470_SPI, status);
...
void L6470_Config(StepperMotorDriverHandle_t *StepperMotorDriverHandle, MotorParameterData_t *MotorParameterData)
{
  ...
  */
  ...
  StepperMotorDriverHandle->Register.MAX_SPEED = Step_s_2_MaxSpeed(MotorParameterData->maxspeed);
  StepperMotorDriverHandle->Register.MIN_SPEED = Step_s_2_MinSpeed(MotorParameterData->minspeed);
  StepperMotorDriverHandle->Register.FS_SPD = 0xFF;		 //Step_s_2_FsSpd(MotorParameterData->fsspd);
  StepperMotorDriverHandle->Register.KVAL_HOLD = 0x29; //(uint8_t)((float)((float)(MotorParameterData->kvalhold * 256) / (MotorParameterData->motorvoltage)));
  StepperMotorDriverHandle->Register.KVAL_RUN = 0x29; //(uint8_t)((float)((float)(MotorParameterData->kvalrun * 256) / (MotorParameterData->motorvoltage)));
  StepperMotorDriverHandle->Register.KVAL_ACC = 0x29; //(uint8_t)((float)((float)(MotorParameterData->kvalacc * 256) / (MotorParameterData->motorvoltage)));
  StepperMotorDriverHandle->Register.KVAL_DEC = 0x29; //(uint8_t)((float)((float)(MotorParameterData->kvaldec * 256) / (MotorParameterData->motorvoltage)));
  StepperMotorDriverHandle->Register.INT_SPEED = 0x408;	//Step_s_2_IntSpeed(MotorParameterData->intspeed);
  StepperMotorDriverHandle->Register.ST_SLP = 	0x75;	//s_Step_2_StSlp(MotorParameterData->stslp);
  StepperMotorDriverHandle->Register.FN_SLP_ACC = 0x75; 	//s_Step_2_FnSlpAcc(MotorParameterData->fnslpacc);
  StepperMotorDriverHandle->Register.FN_SLP_DEC = 0x75;	//s_Step_2_FnSlpDec(MotorParameterData->fnslpdec);
  StepperMotorDriverHandle->Register.K_THERM = MotorParameterData->kterm;
  StepperMotorDriverHandle->Register.OCD_TH = mA_2_OcdTh(MotorParameterData->ocdth);
  StepperMotorDriverHandle->Register.STALL_TH = mA_2_StallTh(MotorParameterData->stallth);
  StepperMotorDriverHandle->Register.STEP_MODE = MotorParameterData->step_sel;
